...
export default function InternshipCard({ internship, isSaved, isSelected, onClick, handleSave }: InternshipCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className={`bg-white rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer relative group overflow-hidden shrink-0 ${isSelected ? 'border-2 border-praksonar-gold' : 'border border-gray-200 hover:border-praksonar-teal/30'
                }`}
        >
            {/* Selected Indicator Bar */}
            {isSelected && (
                <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-praksonar-gold"></div>
            )}

            <div className="p-5 flex flex-col sm:flex-row gap-4">
                {/* Main Info */}
                <div className="flex-1">
                    <p className="text-sm font-medium text-praksonar-teal/70 mb-1">{internship.company}</p>
                    <h3 className="text-lg font-medium text-gray-900 leading-tight mb-3 line-clamp-2 pr-8">{internship.title}</h3>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        ...
                        {internship.source_name && (
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-praksonar-mint text-praksonar-teal">
                                {internship.source_name}
                            </span>
                        )}
                ...
                <div className="flex sm:flex-col justify-between items-end sm:items-end mt-2 sm:mt-0 sm:min-w-[120px]">
                    ...
                    >
                        <svg className="w-6 h-6 transition-transform hover:scale-110" fill={isSaved ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="text-right w-full sm:w-auto">
                        ...
                        <button className={`px-4 py-2 w-full sm:w-auto rounded-md text-sm font-medium transition-colors ${isSelected ? 'bg-praksonar-gold text-white shadow-sm' : 'bg-praksonar-teal text-white hover:bg-praksonar-teal/90'
                            }`}>
                            Pogledaj
                        </button>
                    </div>
